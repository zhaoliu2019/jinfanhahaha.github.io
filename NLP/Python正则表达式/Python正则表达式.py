...
match = pattern.match('hello, chixigua! How are you?')
...
'''Match:
   ...
   方法：
      ...
      expand(template): 
        将匹配到的分组代入template中然后返回。template中可以使用\id或\g、\g引用分组，但不能使用编号0。\id与\g是等价的；但\10将被认为是第10个分
        组，如果你想表达\1之后是字符'0'，只能使用\g<1>0。'''
import re
m = re.match(r'(\w+) (\w+)(?P<sign>.*)', 'hello chixigua!')
...
# 这个例子中使用match()无法成功匹配 
match = pattern.search('hello chixigua!') 
...
'''sub(repl, string[, count]) | re.sub(pattern, repl, string[, count]):
     ...
     当repl是一个方法时，这个方法应当只接受一个参数（Match对象），并返回一个字符串用于替换（返回的字符串中不能再引用分
     组）。 count用于指定最多替换次数，不指定时全部替换。'''
import re
 
p = re.compile(r'(\w+) (\w+)')
s = 'i say, hello chixigua!'
